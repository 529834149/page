 ...
 * 初始化SCCB所需引脚
 */
static void LPLD_SCCB_Init(void)
{
 ...
 *    0-失败
 */
static uint8 LPLD_SCCB_WriteReg(uint8 reg_addr, uint16 data)
{		
  if(!LPLD_SCCB_Start())
  {
    return 0;
 ...
 *    0-失败
 */          
static uint8 LPLD_SCCB_ReadReg(uint8 reg_addr, uint16* data)
{
  uint8 data_l;
  if(!LPLD_SCCB_Start())
  {
...
void MT9V034_Init()
{
  LPLD_SCCB_Init();
  Delayms(20);
  LPLD_SCCB_WriteReg(MTV_READ_MODE_REG_A,0x030a);  
